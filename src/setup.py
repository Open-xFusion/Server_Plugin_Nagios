...
class CheckCfgTask:
    ...
    def checkPath(self, pathStr):
        result = self.isDir(pathStr) or self.isFile(pathStr) or self.isExists(
            pathStr)
        if not result:
            self.LOG.error(
    ...
    def checkPathNot(self, pathStr):
        ...
        if result:
            self.LOG.error(
                'checkpath:' + pathStr + (result and '  fail.' or '  sucess.'))
        return result
...
def addckmkVersionToprofile():
    ...
    try:
        ...
        strlist = file.readlines()
        file.writelines(
            strforconfig + ("%s" % CKmkVersion.getckmkVersion()) + "\n")
...
class FileMrg:
    ...
    def readEditFile(self, nagiosCfgPath, textContent, regIndexContent=None,
                     regPtContent=None, mode=True):
        """
        '如果更改的文件内容不是一个路径
        """
...
class TargetPath(object):
    ...
    def nagios_cache_path(self):
        return self.NAGIOS_CACHE_PATH or '%s/XFUSION_server' % self.nagios_var()
    ...
    def nagios_XFUSION_config(self):
        return '%s/XFUSION_server/templates/XFUSION_server.cfg.template' % self.nagios_etc()
    
    def nagios_XFUSION_command(self):
        return '%s/XFUSION_server/templates/XFUSION_command.cfg.template' % self.nagios_etc()
    
    def nagios_XFUSION_ser_cfg(self):
        return '%s/XFUSION_server/XFUSION_server.cfg' % self.nagios_etc()
    
    def nagios_XFUSION_listener(self):
        return '%s/XFUSION_server/XFUSION_listener.cfg' % self.nagios_etc()
...
class InitUserCfg(object):
    def initXFUSIONServer(self):
        initXFUSIONSer = '''<?xml version="1.0" encoding="UTF-8"?>
<hosts>
        <host>
...
'''
        fileMrg.createFile(targetPath.CURR_PATH + '/etc/XFUSION_server',
                           'XFUSION_hosts.xml', initXFUSIONSer, 'w')
    
    def initXFUSIONHosts(self):
        fileMrg.removeFile(
            targetPath.CURR_PATH + '/etc/XFUSION_server/XFUSION_server.cfg')
        pass
    
    def initCfg(self):
        self.initXFUSIONServer()
        self.initXFUSIONHosts()
...
def createPluginDir():
    ...
    if not os.path.isdir(targetPath().nagios_var() + "/" + "XFUSION_server"):
        os.mkdir(targetPath().nagios_var() + "/" + "XFUSION_server")
...
def moveFile():
    ...
    fileMrg.copyFile(
        targetPath.CURR_PATH + os.path.sep + 'bin' + os.path.sep + 'XFUSION_server',
        targetPath().nagios_bin())
    fileMrg.copyFile(
        targetPath.CURR_PATH + os.path.sep + 'etc' + os.path.sep + 'XFUSION_server',
        targetPath().nagios_etc())
    fileMrg.copyFile(
        targetPath.CURR_PATH + os.path.sep + 'libexec' + os.path.sep + 'XFUSION_server',
...
def removeFile(isSaveData):
    ...
    userData = None
    if isSaveData:
        userData = "XFUSION_hosts.xml|XFUSION_server.cfg"
    fileMrg.removeFile(
        targetPath().nagios_bin() + os.path.sep + "XFUSION_server")
    fileMrg.removeFile(
        targetPath().nagios_etc() + os.path.sep + "XFUSION_server", userData)
    fileMrg.removeFile(
        targetPath().nagios_libexec() + os.path.sep + "XFUSION_server")
...
def delInfoInNagiosCfg():
    ...
    if '1_4' in CKmkVersion.getckmkVersion() or '1_5' in CKmkVersion.getckmkVersion():
        ...
        strforconfig = "precached_object_file=%s" % nagioshome + os.path.sep + "etc" + os \
            .path.sep + "XFUSION_server" + os.path.sep + "XFUSION_server.cfg\n"
    else:
        FilePath = nagioshome + os.path.sep + "etc" + os.path.sep + "nagios.cfg"
        strforconfig = "cfg_file=%s" % nagioshome + os.path.sep + "etc" + os. \
            path.sep + "XFUSION_server" + os.path.sep + "XFUSION_server.cfg\n"
    file = None
    try:
        file = open(FilePath, "r+")
...
def reEditFile():
    ...
    XFUSIONCfgPt = 'cfg_file='
    XFUSIONCfg = XFUSIONCfgPt + targetPath().nagios_XFUSION_ser_cfg()
    XFUSIONCfgPt = r'^\s*' + XFUSIONCfg + r'\s*$'
    fileMrg.readEditFile(targetPath().nagios_f_config(), XFUSIONCfg, XFUSIONCfgPt,
                         mode=False)
...
XFUSION_server = 'XFUSION_server.cfg'
XFUSION_command = 'XFUSION_command.cfg'
currentfilepath = os.path.abspath(os.path.dirname(
    this_file)) + os.path.sep + "etc" + os.path.sep + "XFUSION_server"
...
'''
    fileMrg.createFile(targetPath.CURR_PATH + '/etc/XFUSION_server',
                       'initial.cfg', initial_content, 'w')


# verify python vesion is 2.7.13; if yes return python executable path
def VerifyPythonVersion():
    try:
        if str(sys.version_info[0]) == '2' and str(
                sys.version_info[1]) == '7' and str(sys.version_info[2]) == '13':
...
{
    cd `dirname $0`
    nohup python ''' + targetPath().nagios_bin() + os.path.sep + '''XFUSION_server''' + os.path.sep + '''trapd.py > /dev/null 2>&1 &
}
...
{
    cd `dirname $0`
    nohup python ''' + targetPath().nagios_bin() + os.path.sep + '''XFUSION_server''' + os.path.sep + '''collect.py -p > /dev/null 2>&1 &
}

function startCollectByCfg()
{
    cd `dirname $0`
    nohup python ''' + targetPath().nagios_bin() + os.path.sep + '''XFUSION_server''' + os.path.sep + '''collectInfoByCfg.py -p > /dev/null 2>&1 &
}
...
''' % PythonExcutePath
    fileMrg.createFile(targetPath.CURR_PATH + '/libexec/XFUSION_server',
                       'trapdcheck.sh', trapdcheck, 'w')
    
    # 修改启动脚本为nohup python /usr/local/nagios/bin/XFUSION_server/trapd.py > /dev/null &
...
def encrypt(str):
    ...
    """
    encryptStr = os.popen(
        "echo " + "'" + str + "'" + " | openssl aes-128-cbc -k '' -base64") \
        .read().strip()
...
def install():
    ...
    os.system(
        "trapdNum=`ps -ef | grep trapd.py|grep python | grep -v grep | awk "
        "'{print $2}'` && if [ ! -e $trapdNum ]; then kill -9 $trapdNum; fi")
    os.system(
    ...
    if ("__main__" == __name__) and checkpath():
        if checkCfgTask.checkPathNot(
            ...
            LOG.error(
                'Plug-in registrationed.Please uninstalled, try install.')
            raise InstallExcept()
        ...
        moveFile()
        os.system(
            "chmod  -R 700 " + targetPath().nagios_var() + os.path.sep + "XFUSION_server")
        os.system(
            "chmod  -R 700 " + targetPath().nagios_bin() + os.path.sep + "XFUSION_server")
        os.system(
            "chmod  -R 700 " + targetPath().nagios_libexec() + os.path.sep + "XFUSION_server")
        os.system(
            "chmod  -R 700 " + targetPath().nagios_etc() + os.path.sep + "XFUSION_server")
        
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "configInfo.cfg")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "XFUSION_hosts.xml")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "initial.cfg")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "XFUSION_plugin*.cfg")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "pluginConfig.xml")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "device.xml")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "device" + os.path.sep + "High-density_Common*.xml")
        os.system(
            "chmod  600 " + targetPath().nagios_etc() + os.path.sep +
            "XFUSION_server" + os.path.sep + "device" + os.path.sep + "Rack_Common*.xml")
        ...
        if CKmkVersion.getckmkVersion() == '1_4' or CKmkVersion.getckmkVersion() == '1_5':
            ...
            copyusrfilecmd = "cp  '%s'/usrFile.cfg '%s'" % (_currenPath,
                                                            targetPath().nagios_etc() + os.path.sep + "XFUSION_server")
            os.system(copyusrfilecmd)
            os.system("chown -R %s:%s %s" % (
                G_CMK_USER, G_CMK_GROUP, targetPath.NAGIOS_HOME))
        
        else:
            os.system("chown -R %s:%s " % (G_NAGIOS_USER,
                                           G_NAGIOS_GROUP) + targetPath().
                      nagios_bin() + os.path.sep + "XFUSION_server")
            os.system("chown -R %s:%s " % (G_NAGIOS_USER,
                                           G_NAGIOS_GROUP) + targetPath().
                      nagios_etc() + os.path.sep + "XFUSION_server")
            os.system("chown -R %s:%s " % (G_NAGIOS_USER,
                                           G_NAGIOS_GROUP) + targetPath().
                      nagios_libexec() + os.path.sep + "XFUSION_server")
            os.system("chown -R %s:%s " % (G_NAGIOS_USER,
                                           G_NAGIOS_GROUP) + targetPath().
                      nagios_var() + os.path.sep + "XFUSION_server")
        # set ENV
...
def uninstall(isRetain):
    ...
    if checkpath():
        ...
        os.system(
            "trapdNum=`ps -ef | grep trapd.py |grep python| grep -v grep | "
            "awk '{print $2}'` && if [ ! -e $trapdNum ]; then kill -9 "
            "$trapdNum; fi")
        os.system(
            "collectNum=`ps -ef | grep collect.py|grep python | grep -v grep "
            "| awk '{print $2}'` && if [ ! -e $collectNum ]; then kill -9 "
...
def main():
    ...
    optParser.add_option("-s", "--retain", action="store", dest="retain",
    ...
    if len(args) == 1 and (args[0] == 'install' or args[0] == 'uninstall'):
        if args[0] == 'uninstall':
            ...
            try:
                if '1_4' in CKmkVersion.getckmkVersion() or '1_5' in CKmkVersion.getckmkVersion():
                    targetPath.NAGIOS_HOME = re.sub(r'[/|\\]*$', '',
                                                    options.nagios_dir) + "/" + "nagiosPlugInForXFUSION"
