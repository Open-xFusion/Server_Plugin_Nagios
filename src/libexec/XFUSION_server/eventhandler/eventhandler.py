...
logger = logging.getLogger('XFUSION_plugin.handler')
...
class HandlerFactory(object):
    ...
    def createHandler(agentip, varBinds, configdata):
        ...
        if agentip not in configdata.Hostdata.keys():
...
class FileHandler(object):
    ...
    def saveWarningFile(self, trapdata):
        ...
        filepath = self._constructFullFilename(trapdata,
                                               self._constructFilename(
                                                   trapdata))
        file = None
        try:
            info = trapdata.Info
            if trapdata.Blade is not None and '' != trapdata.Blade:
            ...
            raise PluginException('Unknown error when writing warning file. '
                                  'Cause: \n' + str(err))
        finally:
            file.close()
...
class NagiosCorresponder(object):
    ...
    def _constructMessage(self, trapdata, status, info):
        ...
        res = "%s%s%s %s%s%s%salarm%s%s%s%s\n" % (
            NAGIOS_CMD_TIMESTAMP_SURROUND_LEFT, str(time.time()),
            NAGIOS_CMD_TIMESTAMP_SURROUND_RIGHT, NAGIOS_CMD_SIGN,
            NAGIOS_CMD_SEP, trapdata.getHostName(), NAGIOS_CMD_SEP,
            NAGIOS_CMD_SEP, str(status), NAGIOS_CMD_SEP, info)
        return res
...
class TrapData(object):
    ...
    def matchingEventCode(self, eventcode, eventcodematching):
        ...
        if eventcodematching is not None:
    ...
    def getStatus(self):
        ...
        """
        if not re.match(STATUS_OK_STR_PATTERN + "|" + STATUS_NORMAL_PATTERN +
                        "|" + STATUS_INFO_PATTERN, self._status) is None:
    ...
    def _parseBindingValuesMsg(self, varBinds):
        ...
        for oid, val in varBinds:
            if oid.prettyPrint() == self._configdata.getBindingValuesOID() or \
                    oid.prettyPrint() == self._configdata.getBindingValuesOIDV5():
    ...
    def _parseBindingValues(self, standardmsg):
        ...
        bindingpattern = (TRAP_BINDING_VALUE_SEP + " *"). \
            join([BINDING_VALUE_PATTERN for i in range(propertycount)])
    ...
    def _preHandlerE6000(self, bindingvalues):
        ...
        propertycount = 8
        bindingpattern = \
            '(Time):(.*)%s *(Location):(.*)%s *(Chassis Serial # ):(.*)%s *(' \
            'Blade):(.*)%s *(Sensor):(.*)%s *(Event):(.*(' \
            'Assertion:|Deassertion,)(.*))%s *(Event Code ):(.*)' \
            % (TRAP_BINDING_VALUE_SEP, TRAP_BINDING_VALUE_SEP,
               TRAP_BINDING_VALUE_SEP, TRAP_BINDING_VALUE_SEP,
               TRAP_BINDING_VALUE_SEP, TRAP_BINDING_VALUE_SEP)
        ...
        for i in range(propertycount):
            ...
            if key.find(':') != -1 or key.find(',') != -1:
                value = '%s [%s]' % (key.replace(':', '').replace(',', '')
                                     .strip(), value.strip())
                key = 'Severity'
            trapdic[key.strip()] = value.strip()
        logger.info('PreHandle trap of E6000 complete. ')
        return 'Time:%s,Location:%s,Chassis Serial#:%s,Blade:%s,Sensor:%s,' \
               'Event:%s,Severity:%s,Event Code:%s' \
