...
# -------------- OID---------------------------------------
OID_SYSTEM_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.1.1.0"
OID_SHELF_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.2.5.0"
OID_SMM_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.3.9.0"
# '''upload need BULK '''
OID_FAN_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.5.2001.1.7"
OID_FAN_PRESENCE = "1.3.6.1.4.1.58132.2.82.1.82.5.2001.1.2"
OID_POWER_PRESENCE = "1.3.6.1.4.1.58132.2.82.1.82.6.2001.1.2"
OID_POWER_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.6.2001.1.3"
# ''' OID NEED HANDLE SPECIALLY'''
OID_BLADE_PRESENCE = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.6.0"
OID_BLADE_STATUE = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.8.0"
OID_BLADE_INFO = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.39.0"

OID_BLADE_CPU_PRESENT = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2006.1.4"
OID_BLADE_CPU_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2006.1.5"
OID_BLADE_MEMORY_PRESENT = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2007.1.4"
OID_BLADE_MEMORY_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2007.1.5"
OID_BLADE_MEZZ_PRESENT = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2008.1.4"
OID_BLADE_MEZZ_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2008.1.5"
OID_BLADE_DISK_PRESENT = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2009.1.4"
OID_BLADE_DISK_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.2009.1.5"
OID_BLADE_RAID_PRESENT = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.58132.1.2"
OID_BLADE_RAID_NAME = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.58132.1.3"
OID_BLADE_RAID_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.4.%s.58132.1.7"
BLADE_CPU = "CPU"
BLADE_MEMORY = "MEMORY"
BLADE_MEZZ = "MEZZ"
BLADE_DISK = "DISK"
...
OID_SWI_PRESENCE = "1.3.6.1.4.1.58132.2.82.1.82.7.%s.6.0"
OID_SWI_STATUS = "1.3.6.1.4.1.58132.2.82.1.82.7.%s.8.0"
...
def commandParse():
    ...
    compenlist = [COMPONENT_POWER, COMPONENT_FAN, COMPONENT_SYS, COMPONENT_SWI,
                  COMPONENT_SMM, COMPONENT_SHELF, COMPONENT_BLADE]
...
class InfoHandler():
    ...
    def outputStatus(self):
        ...
        print "%s HealthStatus: %s " % (
            str(self._Parm.component), MSG_HEASTATUS[_status])
        print "=============================== info ============================="
        print _infoStr
        return _status
    ...
    def snmpGet(self, oid):
        ...
        if self._Parm.version == '3':
            ...
            _comStr = "snmpget  -t %s -r %s -v %s -c %s  %s:%s %s" \
                      % (self._Parm.timeout, self._Parm.retry, self._Parm.version,
                         self._Parm.community, self._Parm.host,
                         self._Parm.port, oid)
    ...
    def getBladeStatus(self):
        ...
        for _item in _presentIndexList:
            ...
            _info = _info + "blade " + str(_item) + ":" + MSG_HEASTATUS[
                _healthStatus] + "\n" + _baldecpuInfo + _bladeMemoryInfo +\
                _bladeDiskInfo + _bladeMezzInfo + _bladeRaidInfo + "\n"
    ...
    def getBladeAllInfo(self, bladeNo, component):
        ...
        '''
        _tempListpresent = []
        _ret, _output = self.snmpWalk(
            DIS_BLADE_INFO_OID[component][1] % str(bladeNo))
        ...
        _ret, _output = self.snmpWalk(
            DIS_BLADE_INFO_OID[component][0] % str(bladeNo))
        ...
        _presentMessage = ("%s presentStatus:" % component) + str(
            len(_presentIndexList)) + '/' + str(_totalcnt) + "\n"
        _satatusMesssage = ''
        for _item in _presentIndexList:
            _satatusMesssage = _satatusMesssage + ("%s" % component) + str(
                _item) + " status: " \
                               + self.replacebaldeStatu(_tempListstatus[_item],
                                                        component) + '; '
    ...
    def getPowerStatus(self):
        ...
        for i in range(_totalcnt):
            ...
            if _tempListpresent[i] == "1":
                _presentIndexList.append(i)
        _presentMessage = "presentStatus:" + str(
            len(_presentIndexList)) + '/' + str(_totalcnt) + "\n"
        _satatusMesssage = ''
        _status = STATUS_OK
        for _item in _presentIndexList:
            _satatusMesssage = _satatusMesssage + 'power' + str(
                _item) + " status: " \
    ...
    def getFanStatus(self):
        ...
        _status = STATUS_OK
        for _item in _presentIndexList:
            _satatusMesssage = _satatusMesssage + 'fan' + str(
                _item) + " status: " \
                               + _dis[_tempListstatus[_item]] + '; '
            if "2" == _tempListstatus[_item]:
                _status = STATUS_CRITICAL
            if "5" == _tempListstatus[_item] and (
                    not (_status == STATUS_CRITICAL)):
                _status = STATUS_WARNING
        _info = _presentMessage + _satatusMesssage + '\n'
        return _status, _info
    ...
    def replaceStatus(self, statusin):
        ...
        _dis = {'0': STATUS_OK, '1': STATUS_WARNING, '2': STATUS_CRITICAL,
                '3': STATUS_CRITICAL, '4': STATUS_CRITICAL,
                '5': STATUS_CRITICAL,
    ...
    def getSwiStatus(self):
        ...
        for swiNo in _presentIndexList:
            ...
            _info = _info + "switch" + str(swiNo) + " status:" + MSG_HEASTATUS[
                _tmpstatus] + "; "
