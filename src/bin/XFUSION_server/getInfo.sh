...
if [ $# == 2 ];then
    if [ $1 == "-H" ];then
        hostip=$2
        cmd="python /usr/local/nagios/bin/XFUSION_server/collect.py -p 2>&1 | grep ${hostip}| grep \;system\;"
    else
    ...
    cmd="python /usr/local/nagios/bin/XFUSION_server/collect.py -p 2>&1 | grep ${hostip} | grep \;${device}\;"
else
   echo "please input command as:getInfo.sh -H 192.168.3.1 -S cpu/memory/hardDisk/fan/power/system"
   exit 3
fi
