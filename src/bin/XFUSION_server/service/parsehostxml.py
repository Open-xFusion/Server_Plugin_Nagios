...
class ParseHostXml:
    ...
    def parseHostXml(cls, hostPath, hosts):
        ...
        for node in Common.getChild(root):
            if "host" != node.tag:
                cls._logger.error(
                    "parser host xml: child node name is not host.")
                return False

            host = Host()
            if not cls.__parseHostNode(node, host):
                cls._logger.error(
                    "parser host xml: host node format is not valid.")
                return False
            hosts.append(host)
        return True
    ...
    def __parseHostNode(cls, node, host):
        ...
        for element in elements:
            if "device" == element.tag:
                ...
                cls._logger.error(
                    "parser host node: host node should not has %s node." % element.tag)
                return False
    ...
    def __parseDeviceNode(cls, node, host):
        ...
        if elements is None:
            cls._logger.error(
                "parser device node: device node has no child node.")
            return False
        ...
        for element in elements:
            ...
            elif "vendorid" == element.tag:
                host.setVendorId(element.text.strip())
            elif "port" == element.tag:
                host.setPort(element.text.strip())
            else:
                cls._logger.error(
                    "parser device node: device node should not has %s node." % element.tag)
                return False
    ...
    def __parseCollectNode(cls, node, host):
        ...
        if elements is None:
            cls._logger.error(
                "parser collect node: collect node has no child node.")
            return False
        ...
        for element in elements:
            if "snmpversion" == element.tag:
                ...
                cls._logger.error(
                    "parser collect node: collect node should not has %s node." % element.tag)
                return False
        host.setCollectBasic(
            cls.__getDevcieCollect(host.getDeviceType(), "basic"))
        host.setCollectExtension(
            cls.__getDevcieCollect(host.getDeviceType(), "extension"))
        return True
    ...
    def __parseAlarmNode(cls, node, host):
        ...
        if elements is None:
            cls._logger.error(
                "parser alarm node: alarm node has no child node.")
            return False
    ...
    def __getDevcieCollect(cls, deviceType, collectType):
        ...
        if "Rack" == deviceType:
            if "basic" == collectType:
                ...
                return ["system", "power", "fan", "cpu", "memory", "hardDisk",
                        "pCIe", "raid", "logical", "component", "sensor",
                        "firmwareVersion"]
            ...
            cls._logger.error(
                "get device collect failed, device type:%s." % deviceType)
            return []
    ...
    def __readKey(cls):
        file = None
        try:
            kfilepath = Common.getEtcPath() + 'XFUSION_server' + os.path.sep + 'configInfo.cfg'
    ...
    def __genRootKeyStr(cls):
        configfilepath = Common.getEtcPath() + 'XFUSION_server' + os.path.sep + 'initial.cfg'
        ...
        try:
            ...
            cls._logger.exception(
                "read root key: get root key exception:%s." % e)
            return ""
        finally:
            if file is not None:
                file.close()
