...
class Check:
    ...
    def checkFileModeParam(cls, params):
        if NUMBER_ONE == len(params):
            if os.path.isfile(params[0]) and \
                ...
                cls._logger.error("check file mode: the host config file "
                                  "is invalid when param len is two")
                return True
    ...
    def checkTargetParam(cls, params):
        if "-r" != params[0]:
            cls._logger.error("check target param: the second command is not "
                              "-r.")
            return False
        if not os.path.isdir(params[1]):
            cls._logger.error(
                "check target param: the target path is invalid.")
            return False
        return True
