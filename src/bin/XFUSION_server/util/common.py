...
from constant.constant import NAGIOS_CMD
...
class Common:
    ...
    def getEtcPath(cls):
        return os.path.dirname(
            cls.getBinPath()) + os.path.sep + "etc" + os.path.sep

    # 获取var路径
    @classmethod
    def getVarPath(cls):
        return os.path.dirname(
            cls.getBinPath()) + os.path.sep + "var" + os.path.sep

    # 获取nagios cmd路径
    @classmethod
    def getNagiosCmdPath(cls):
        nagioscmd_file = ''
        initial_cfg = open(os.path.dirname(
            cls.getBinPath()) + os.path.sep + "etc" + os.path.sep + "XFUSION_server/initial.cfg")
    ...
    def getPluginConfigPath(cls):
        return cls.getEtcPath() + "XFUSION_server" + os.path.sep + "pluginConfig.xml"

    # 获取Host配置文件路径
    @classmethod
    def getHostConfigPath(cls):
        return cls.getEtcPath() + "XFUSION_server" + os.path.sep + "XFUSION_hosts.xml"

    # 获取设备配置文件路径
    @classmethod
    def getDeviceConfigPath(cls):
        return cls.getEtcPath() + "XFUSION_server" + os.path.sep + "device.xml"

    # 获取具体设备配置文件路径
    @classmethod
    def getDeviceModelPath(cls):
        return cls.getEtcPath() + "XFUSION_server" + os.path.sep + "device"
    ...
    def getFilePath(cls, path, hostName, ipAddress):
        if not os.path.exists(path):
            os.makedirs(path)
        return path + os.path.sep + datetime.datetime.now().strftime(
            '%Y%m%d%H%M%S%f') + "_" + hostName + " (%s)" % ipAddress + ".xml"
    ...
    def writeCmd(cls, message):
        ...
        try:
            ...
            cls._logger.exception(
                "write cmd: option file object exception:" % e)
            return False
        finally:
            if 'file' in locals():
                file.close()
    ...
    def constructMessage(cls, hostName, serviceName, status, information):
        return NAGIOS_CMD % (
            str(time.time()), hostName, serviceName, status, information)
